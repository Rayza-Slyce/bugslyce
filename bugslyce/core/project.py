...
def build_project_state(input_dir: Path) -> ProjectState:
    ...
    raw = assemble_raw_artifacts(
        input_dir=input_dir,
        manifest=manifest,
        default_host=_default_raw_host(scope, subdomains, manifest),
        evidence=evidence,
...
def _default_raw_host(
    scope: ParsedScope,
    subdomains: list[ParsedSubdomain],
    manifest: ReconManifest | None = None,
) -> str | None:
    if manifest and manifest.target:
        parsed = urlparse(manifest.target if "://" in manifest.target else f"//{manifest.target}")
        return normalise_hostname(parsed.hostname or manifest.target)
