...
def _build_parser() -> argparse.ArgumentParser:
    ...
    deep_source_route_coverage_parser = recon_subparsers.add_parser(
        "deep-source-route-coverage",
        help=(
            "Preview offline Deep source/route coverage from existing local "
            "evidence without running Deep Recon."
        ),
        description=(
            "Preview offline Deep source/route coverage from existing local "
    ...
    deep_source_route_collect_parser = recon_subparsers.add_parser(
        "deep-source-route-collect",
        help=(
            "Collect bounded Deep source/route responses from an existing "
...
def _recon(args: argparse.Namespace) -> int:
    ...
    if args.recon_command == "deep-source-route-coverage":
        if not args.input_dir.exists():
            print(f"Error: input directory does not exist: {args.input_dir}", file=sys.stderr)
            print("No files were written.", file=sys.stderr)
            print("No network requests were made.", file=sys.stderr)
    ...
    if args.recon_command == "deep-source-route-collect":
        if not args.input_dir.exists():
            print(f"Error: input directory does not exist: {args.input_dir}", file=sys.stderr)
        ...
        try:
            ...
            print(
                f"Error: could not build Deep source/route collection plan: {exc}",
                file=sys.stderr,
            )
            _print_deep_source_route_collection_guardrails()
            return 2
