...
def test_recon_mode_registry_has_deterministic_order() -> None:
    ...
    assert [mode.mode_id for mode in modes] == ["deep"]
    assert [mode.display_name for mode in modes] == ["Reconnaissance"]


def test_quick_recon_is_not_an_executable_operator_mode() -> None:
    assert QUICK_RECON_PROFILE == "lab-safe-tiny"
    with pytest.raises(ValueError, match="Unknown recon mode"):
        resolve_executable_profile("quick")


def test_standard_recon_is_not_an_executable_operator_mode() -> None:
    assert STANDARD_RECON_PROFILE == "standard-bounded"
    with pytest.raises(ValueError, match="Unknown recon mode"):
...
def test_deep_recon_is_available_and_maps_to_deep_bounded() -> None:
    ...
    assert deep.display_name == "Reconnaissance"
...
def test_removed_modes_do_not_fall_back_to_reconnaissance() -> None:
    for mode_id in ("quick", "standard"):
        with pytest.raises(ValueError, match="Unknown recon mode"):
            get_recon_mode(mode_id)
