...
def test_recon_mode_menu_uses_user_facing_names() -> None:
    ...
    assert QUICK_SAFE_RECON_LABEL in menu
    assert "Manual Setup Only" in menu
    assert "Standard Safe Recon" in menu
...
def test_manual_setup_only_scaffolds_and_shows_next_without_pipeline(
    ...
    monkeypatch.setattr(
        ...
        lambda path: calls.append(f"next:{path}") or SimpleNamespace(),
    )
    monkeypatch.setattr(
        "bugslyce.interactive.render_project_next",
        lambda result: "NEXT PREVIEW",
    )
    monkeypatch.setattr(
    ...
    assert exit_code == 0
    assert calls == ["scaffold", f"next:{project_file}"]
    assert "SCAFFOLD SUMMARY" in output
    assert "NEXT PREVIEW" in output


def test_quick_safe_recon_run_now_calls_pipeline(monkeypatch, tmp_path: Path) -> None:
    project_file = tmp_path / "projects" / "demo" / "bugslyce_project.json"
...
def test_quick_safe_recon_no_run_shows_command_preview(
    monkeypatch,
    tmp_path: Path,
) -> None:
    project_file = tmp_path / "projects" / "demo" / "bugslyce_project.json"
...
def test_resume_no_shows_command_preview(monkeypatch, tmp_path: Path) -> None:
    ...
    output: list[str] = []
    inputs = iter(["2", str(project_file), "no"])
