...
This guide is the current acceptance procedure for BugSlyce `1.1.0`. The
...
Both commands must report `1.1.0`; the CLI form must print `bugslyce 1.1.0`.
...
```bash
WHEEL=/absolute/path/to/bugslyce-1.1.0-py3-none-any.whl
sha256sum "$WHEEL"
PIPX_HOME=$(mktemp -d)
...
Expected version: `bugslyce 1.1.0`.
